...
static ListResultItem_t* GetProperties(struct udev_device* dev, ListResultItem_t* item) {
	...
	usbattr = udev_device_get_sysattr_value(dev,"idVendor");
	if(usbattr)
		item->vendorId = strtol(usbattr, NULL, 16);
	usbattr = udev_device_get_sysattr_value(dev,"idProduct");
	if(usbattr)
		item->productId = strtol(usbattr, NULL, 16);
